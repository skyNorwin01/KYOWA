Public Class FrmTrucker
    ...
    Private Sub FrmTrucker_Load(sender As Object, e As EventArgs) Handles MyBase.Load, txtparticular.TextChanged
        With dtList
            Dim Dbo As New SQLClass
            Dbo.SqlCon.Open()
            SQL = "SELECT ID, TRUCKER FROM TBL_TRUCKER WHERE STAT = '1' AND TRUCKER LIKE '" & KVal(txtparticular.Text) & "%' AND TRUCKER <> '' ORDER BY TRUCKER ASC"
    ...
    Private Sub btnSave_Click(sender As Object, e As EventArgs) Handles btnSave.Click
        If ModeIs = "NEW" Then
            If MsgBox("Are you sure you want to save this recod?", MsgBoxStyle.Question + MsgBoxStyle.YesNo) = MsgBoxResult.Yes Then
                Call SaveTrucker(KVal(txtparticular.Text), USER_ID, Now, "1")
                Call FrmTrucker_Load(e, e)



                Call LoadStrCmb(FrmInitialBooking.cmbTrucker, "SELECT ID, TRUCKER FROM TBL_TRUCKER WHERE STAT = '1'")
        ...
        If ModeIs = "EDIT" Then
            If MsgBox("Are you sure you want to save changes", MsgBoxStyle.Question + MsgBoxStyle.YesNo) = MsgBoxResult.Yes Then
                Call SetJob("UPDATE TBL_TRUCKER SET TRUCKER = '" & KVal(txtparticular.Text) & "', AddedBy = '" & USER_ID & "', DateAdded = '" & Now & "' WHERE STAT = '1' AND  ID =  '" & txtparticular.Tag & "'")
                Call FrmTrucker_Load(e, e)
                Call LoadStrCmb(FrmInitialBooking.cmbTrucker, "SELECT ID, TRUCKER FROM TBL_TRUCKER WHERE STAT = '1'")
    ...
    End Sub
    Public Sub SaveTrucker(TRUCKER, AddedBy, DateAdded, STAT)
        Dim Dbo As New SQLClass
        Dbo.SqlCon.Open()
        SQL = "INSERT INTO TBL_TRUCKER(TRUCKER, AddedBy, DateAdded, STAT)"
        SQL = SQL + "VALUES('" & KVal(TRUCKER) & "',  '" & KVal(AddedBy) & "',  '" & KVal(DateAdded) & "',  '" & KVal(STAT) & "')"
        Dbo.SQLCmd = New SqlClient.SqlCommand(SQL, Dbo.SqlCon)
    ...
    Private Sub dtList_CellContentClick(sender As Object, e As DataGridViewCellEventArgs) Handles dtList.CellContentClick
        If e.ColumnIndex = 2 Then
            If MsgBox("Are you sure you want to delete this record?", MsgBoxStyle.Question + MsgBoxStyle.YesNo) = MsgBoxResult.Yes Then
                Call SetJob("UPDATE TBL_TRUCKER SET STAT = '0' WHERE STAT = '1' AND ID = '" & dtList.CurrentRow.Cells(0).Value & "'")
                Call FrmTrucker_Load(e, e)
    ...
    Private Sub FrmTrucker_FormClosing(sender As Object, e As FormClosingEventArgs) Handles MyBase.FormClosing
        Me.Dispose()
    End Sub
End Class